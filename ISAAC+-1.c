...
void isaac_plus() {
    ...
    cc = cc + 1; /* Increment cc once per 256 results */
    bb = bb + cc; /* Combine cc with bb */
...
void randinit(int flag) {
    ...
    for (i = 0; i < 256; i += 8) { /* fill in mm[] with messy stuff */
        if (flag) {
            ...
            d += randrsl[i + 3]; e += randrsl[i + 4]; f += randrsl[i + 5];
    ...
    if (flag) { /* do a second pass to make all of the seed affect all of mm */
        for (i = 0; i < 256; i += 8) {
            a += mm[i]; b += mm[i + 1]; c += mm[i + 2];
            d += mm[i + 3]; e += mm[i + 4]; f += mm[i + 5];
            g += mm[i + 6]; h += mm[i + 7];
            mix(a, b, c, d, e, f, g, h);
            mm[i] = a; mm[i + 1] = b; mm[i + 2] = c; mm[i + 3] = d;
            mm[i + 4] = e; mm[i + 5] = f; mm[i + 6] = g; mm[i + 7] = h;
    ...
    isaac_plus(); /* fill in the first set of results */
